...
//        throw new UnsupportedOperationException("Not implemented yet.");
    ...
    public <R> ComposableList<R> concatMap(
        ...
        for (T itemInList : this) {
            ...
            // ------------ INSERT CODE HERE! ----------------------------
            ComposableList<R> list = projectionFunctionThatReturnsList.apply(itemInList);
            for (R itemInSubList: list) {
                results.add(itemInSubList);
            }
        }
...
//        throw new UnsupportedOperationException("Not implemented yet.");
    ...
    public static ComposableList<JSON> exercise12() {
        ...
        return movieLists.concatMap(
                movieList -> movieList.videos.concatMap(
                    video -> video.boxarts
                        .filter(
                            boxart -> boxart.height == 200 && boxart.width == 150
                        )
                        .map(
                            boxart -> json("id", video.id, "title", video.title, "boxart", boxart.url)
                        )
                )
        );
    }
    ...
    */
    public static ComposableList<Integer> exercise16() {
        ...
        return ratings.reduce(0, (r, t) -> Math.max(r, t));
    }
    /*
    ...
    public static ComposableList<Map<Integer, String>> exercise18() {
        ...
        return videos.
            ...
            (accumulatedMap, video) -> {
                ...
                // and return the copy.
                // ************ INSERT CODE HERE ************
                accumulatedMap.put(Integer.valueOf(video.id), video.title);
    ...
    */
    public static ComposableList<JSON> exercise20() {
        ...
        for (int counter = 0; counter < Math.min(videos.size(), bookmarks.size()); counter++) {
            ...
            Video video = videos.get(counter);
            Bookmark bookmark = bookmarks.get(counter);
